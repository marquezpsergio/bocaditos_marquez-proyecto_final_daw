...
if (isset($_SESSION['loggedIn']) && ($_SESSION['loggedIn'] == true)) {
  ...
  if ($tiempo_pasado >= 21600) {
    session_destroy();
    echo "<script>location.href = 'extras.php';</script>";
  } else {
...
if (!isset($_SESSION['arrayProdExt']) || !isset($_SESSION['arrayCantExt'])) {
...
<body>
  ...
  <div class="container">
    <?php
    // MOSTRAR EXTRAS
    $array_prods = Base::getExtras();
    if (isset($array_prods)) {
      if (sizeof($array_prods) > 0) {
        echo "<div class='row'>
            <div class='col-12'>
              <h2 class='text-warning text-center mt-4 font-weight-bold'>EXTRAS</h4>
            </div>
          </div>";
          ...
          echo "<div class='col mb-3'>
            <form method='get' action='' class='h-100 pb-2'>
              <div class='card h-100 bg-transparent'>
                  <input type='text' name='codigoExtra' value='$codigo' hidden>
                  <input type='text' name='nombreExtra' value='$nombre' hidden>
                  <input type='text' name='precioExtra' value='$precio' hidden>
                  ...
                  <div class='align-bottom mt-0'>
                    ...
                    <p class='text-white'><strong>Cantidad:</strong> <input type='number' name='cantidadExtra' class='w-25 text-center' min='1' max='200' value='1'></p>
    ...
    if (isset($_REQUEST['addCarrito'])) {
      $codigo = $_REQUEST['codigoExtra'];
      $cant = $_REQUEST['cantidadExtra'];
      $prodExistente = false;
      for ($i = 0; $i < sizeof($_SESSION['arrayProdExt']); $i++) {
        if ($_SESSION['arrayProdExt'][$i] == $codigo) {
          $cantAnterior = $_SESSION['arrayCantExt'][$i];
          $newCant = $cantAnterior + $cant;
          $_SESSION['arrayCantExt'][$i] = $newCant;
          $prodExistente = true;
        }
      }
      if ($prodExistente == false) {
        array_push($_SESSION['arrayProdExt'], $codigo);
        array_push($_SESSION['arrayCantExt'], $cant);
        $_SESSION['cantidadCarrito'] += 1;
      }
      echo "<script>alert('Extra añadido al carrito con éxito.')</script>";
    }
