...
if (isset($_SESSION['loggedIn']) && ($_SESSION['loggedIn'] == true)) {
  ...
  if ($tiempo_pasado >= 21600) {
    session_destroy();
    echo "<script>location.href = 'bocadillos.php';</script>";
  } else {
...
if (!isset($_SESSION['arrayProdBoc']) || !isset($_SESSION['arrayCantBoc'])) {
...
<body>
  ...
  <div class="container">
    <?php
    // MOSTRAR BOCADILLOS
    $array_prods = Base::getBocadillos();
    if (isset($array_prods)) {
      if (sizeof($array_prods) > 0) {
        echo "<div class='row'>
            <div class='col-12'>
              <h2 class='text-warning text-center mt-4 font-weight-bold'>BOCADILLOS</h4>
            </div>
          </div>";
        ...
        foreach ($array_prods as $ap) {
          ...
          echo "<div class='col mb-3'>
            <form method='post' action='' class='h-100 pb-2'>
              <div class='card h-100 bg-transparent'>
                  <input type='text' name='codigoBocadillo' value='$codigo' hidden>
                  <input type='text' name='nombreBocadillo' value='$nombre' hidden>
                  <input type='text' name='precioBocadillo' value='$precio' hidden>
                  ...
                  <div class='align-bottom mt-0'>
                    ...
                    <p class='text-white'><strong>Cantidad:</strong> <input type='number' name='cantidadBocadillo' class='w-25 text-center' min='1' max='200' value='1'></p>
    ...
    if (isset($_REQUEST['addCarrito'])) {
      $codigo = $_REQUEST['codigoBocadillo'];
      $cant = $_REQUEST['cantidadBocadillo'];
      $prodExistente = false;
      for ($i = 0; $i < sizeof($_SESSION['arrayProdBoc']); $i++) {
        if ($_SESSION['arrayProdBoc'][$i] == $codigo) {
          $cantAnterior = $_SESSION['arrayCantBoc'][$i];
          $newCant = $cantAnterior + $cant;
          $_SESSION['arrayCantBoc'][$i] = $newCant;
          $prodExistente = true;
        }
      }
      if ($prodExistente == false) {
        array_push($_SESSION['arrayProdBoc'], $codigo);
        array_push($_SESSION['arrayCantBoc'], $cant);
        $_SESSION['cantidadCarrito'] += 1;
      }
      echo "<script>alert('Bocadillo añadido al carrito con éxito.')</script>";
    }
    ?>
  </div>
  <!-- // BOCADILLOS -->
